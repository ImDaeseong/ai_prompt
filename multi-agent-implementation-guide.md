...
설계는 `multi-agent-architect.md`를 먼저 사용하고, 구현 단계에서 이 문서를 참고한다.
...
`E:\ai_test\ai_img_video_prompt`는 이미 순차 Agent 파이프라인의 시작점이다.
...
지금 도구에 Agent 프롬프트 구조를 추가하면 1단계 구현이 된다.
