...
[Antigravity 공식 Skills 문서](https://antigravity.google/docs/skills)는 워크스페이스 스킬을 `.agents/skills/<스킬-폴더>/SKILL.md`에 배치하도록 안내한다. 이 저장소의 평평한 `antigravity_test/skills/이름.md` 구조를 그대로 폴더에 복사하는 것과는 다르다(5번 The Metaverse Guy 영상 항목에서 이미 정정한 내용과 동일).

1. 사용할 파일과 참조·스크립트, 원본 재사용 조건을 확인한다.
2. 대상 프로젝트에 스킬별 폴더를 만들고 본문을 `SKILL.md`로 배치한다.
3. 필요한 보조 파일을 포함하고 상대 경로를 맞춘다.
4. `description`에 용도와 사용 시점을 적고, 실제 요청에서 로드되는지 확인한다.

## 8. 결과가 기대와 다를 때

- 먼저 파일과 참조 자료를 실제로 읽었는지 확인한다.
- 역할·입력·출력·완료 조건을 구체화한다.
- 필요한 도구·권한·의존성이 없으면 해당 단계를 미실행으로 표시한다.
- 같은 실패를 반복하면 근거를 남기고 접근을 바꾼다(3번 트러블슈팅과 동일 원칙).

플랫폼 UI·예약·메모리 기능의 상세 동작은 현재 공식 문서와 실제 환경에서 확인한다. 이 가이드는 설치나 모델 실행 결과를 보장하지 않는다.
