...
# files (known clean, since the guard already passes in verify_repo.ps1).
...
# Regression (found live 2026-09-18): Get-Content returns a bare String, not
# an array, for a single-line file. Indexing a String returns one character,
# not the whole line -- $lines[0] on a one-line secret file used to be "T"
# (the first char of "TOKEN = ..."), so the guard silently never checked
# single-line files at all. Plant a real secret in a single-line file and
# confirm the real guard, end-to-end, still catches it.
