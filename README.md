...
> 구조를 그림으로 보려면 [ai-prompt-architecture.html](ai-prompt-architecture.html) 참고 (다운로드/클론 후 로컬에서 열람).
...
`ai_test`(개인 도구 개발 워크스페이스)의 중심 질문이 "AI로 개발을 어떻게 더 잘할까?"라면, 이 저장소(`ai_prompt`)의 중심 질문은 "AI에게 어떻게 지시해야 더 좋은 결과가 나올까?"입니다.
